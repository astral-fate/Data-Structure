...
public void addToMSLLTail(MSLLNode<T> newNode) throws IllegalArgumentException {
    if (isInMSLL(newNode.getKey())) {
        throw new IllegalArgumentException("Node with key '" + newNode.getKey() + "' already exists in MSLL.");
    }
...
public void addCityToSublistAtRear(String sublistKey, String cityName, double latitude, double longitude) throws IllegalArgumentException {
    ...
    if (sublistNode == null) {
        ...
        sublistNode = new MSLLNode<>(sublistKey, latitude, longitude);
        sublistNode.setList(new SLL<>()); // Create a new SLL instance for the sublist
    ...
    City cityObject = new City(cityName, latitude, longitude);
    ...
    public double getDistance(String city1, String city2) throws IllegalArgumentException {
        ...
        double a = sinLat * sinLat + Math.cos(Math.toRadians(latitude1)) * Math.cos(Math.toRadians(latitude2)) * sinLon * sinLon;
        double c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
